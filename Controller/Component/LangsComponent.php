...
class LangsComponent extends Component 
{
  ...
  public function initialize( Controller $controller, $settings = array()) 
  {
    ...
    if( empty( $lang) || strpos( $controller->request->here, '/'. $lang .'/') === false)
    {
      ...
      if( strpos( $controller->request->here, '.css') === false && empty( $controller->request->params ['ext']) && !isset( $controller->request ['admin']) && in_array( $locale, Configure::read( 'Config.languages')))
  		{
